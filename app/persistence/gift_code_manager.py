...
def bulk_add_gift_codes(gift_codes):
    """ Add SCS gift codes to the database. """

    for gift_code in gift_codes:
        code = SCSGiftCodePool(gift_code=gift_code, used=False)
        DB.session.add(code)

    DB.session.commit()


def get_unused_gift_code():
...
def get_unused_gift_code_count():
    """ Returns the number of unused gift codes remaining in the database. """

    return DB.session.query(SCSGiftCodePool).\
        filter(SCSGiftCodePool.used.is_(False)).\
        count()


def mark_gift_code_used(gift_code_id):
