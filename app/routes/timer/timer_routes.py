...
NOT_YET_SOLVED = '—'
NOT_YET_SOLVED_ARRAY = [[NOT_YET_SOLVED, -1]]
...
def __build_user_solves_list(user_results, event_total_solves, scrambles):
    ...
    [displayable_time, solve_id] units, plus the most recent solve's friendly display time. """
    ...
    for i, scramble in enumerate(scrambles):
        for solve in user_results.solves:
            if scramble.id == solve.scramble_id:
                user_solves[i] = [solve.get_friendly_time(), solve.id]
